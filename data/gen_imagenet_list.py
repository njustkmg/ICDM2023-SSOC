...
if __name__ == '__main__':
    ...
    init_classes = range(100)
    novel_num = int(100 * novel_ratio)
    seen_num = 100 - novel_num
    # seen_classes = random.sample(init_classes, seen_num)
    seen_classes = range(seen_num)

    fout_train_label = open('/tic_disk/xiwenjuan/ORCA/data/label_%.1f_%.1f.txt'%(novel_ratio, label_ratio), 'w')
    fout_train_unlabel = open('/tic_disk/xiwenjuan/ORCA/data/unlabel_%.1f_%.1f.txt'%(novel_ratio, label_ratio), 'w')

    folders = folders.split(' ')
    for i, folder_name in enumerate(folders):
        files = glob.glob(os.path.join('/tic_disk/xiwenjuan/data/ImageNet100', folder_name, '*JPEG'))
        for filename in files:
            if i in seen_classes and np.random.rand() < label_ratio:
